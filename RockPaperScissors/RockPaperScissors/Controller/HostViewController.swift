...
class HostViewController: UIViewController {
    ...
    // Variable requaired to togle showing navigation bar
    var naviagtionControllerSetActive = false
    // Autogenerated game-ID
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == Constants.HOST_TO_CHOOSE {
            if let chooseVC = segue.destination as? ChooseViewController {
                // Send data to choosing move screen:
                // ID of game and info that it was sended to next screen from HOST
                chooseVC.ID = newID
                chooseVC.user = "HOST"
            }
        }
    ...
    @IBAction func idTextFieldPressed(_ sender: UITextField) {
        ...
        // Get value of new id when it's changed by user.
        // If user doesnt change the game-ID, initialized value will be used.
        newID = sender.text!
    }
